...
const proxyRoutes: Record<string, ProxyOptions> = {
  ...
  "^/([^/]+)/(?:owner/)?(login|logout|me|warehouses|products|shipments|drivers|notifications|force-password-change|forgot-password|reset-password|manager|storekeeper|analytics|keeper|inventory-movements|reports)(/.*)?(\\?.*)?$": laravelProxy(),
};
