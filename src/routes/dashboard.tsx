...
const calculateAge = (birthday: string | null | undefined): number | undefined => {
  if (!birthday) return undefined;
  const match = /^(\d{4})-(\d{1,2})-(\d{1,2})/.exec(birthday.trim());
      ...
      for (const w of wRes.warehouses) {
        ...
        for (const emp of eRes.employees) {
          ...
          all.push({
            ...
            age: calculateAge(emp.system_user.birthday),
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-white/40 hover:bg-transparent">
                <TableHead>{t("manager.login.username")}</TableHead>
