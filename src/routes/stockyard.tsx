...
export const Route = createFileRoute("/stockyard")({
  ...
  head: () => ({
    meta: [
      { title: "Stockyard — Modern Warehouse Management" },
      {
        name: "description",
        content:
          "Stockyard helps teams manage warehouses, inventory, shipments and subscriptions in one place.",
      },
    ],
  }),
});
...
const FEATURES = [
  { icon: Package, title: "Inventory Tracking", text: "Real-time stock levels across every site." },
  { icon: Truck, title: "Shipments", text: "Inbound and outbound flows, all in one view." },
  { icon: BarChart3, title: "Analytics", text: "Decision-ready KPIs and live dashboards." },
  { icon: Shield, title: "Secure Access", text: "Role-based controls for every team member." },
];
    ...
    <main className="min-h-screen bg-[#F0EBD8] text-[#1D2D44]">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        <Link to="/stockyard" className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold tracking-tight">Stockyard</span>
        </Link>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-12 lg:grid-cols-2 lg:items-center lg:py-20">
        <div>
          <h1 className="text-4xl font-bold leading-tight sm:text-5xl">
            Run every warehouse <span className="text-[#A7B3C3]">like clockwork.</span>
          </h1>
          <p className="mt-4 text-lg text-[#1D2D44]/75">
            Stockyard unifies inventory, shipments and team operations in one elegant workspace —
        ...
        <div className="relative">
          <img
            ...
            loading="lazy"
          />
          <div className="absolute -bottom-4 -left-4 hidden rounded-2xl bg-[#1D2D44] p-4 text-[#F0EBD8] shadow-xl sm:block">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="rounded-2xl border border-[#A7B3C3] bg-white/50 p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-md"
            >
              <div className="flex size-10 items-center justify-center rounded-lg bg-[#1D2D44] text-[#F0EBD8]">
                <f.icon className="size-5" />
              </div>
              <h3 className="mt-4 font-semibold">{f.title}</h3>
              <p className="mt-1 text-sm text-[#1D2D44]/70">{f.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16">
        <div className="rounded-3xl bg-[#1D2D44] p-10 text-center text-[#F0EBD8]">
          <h3 className="text-2xl font-bold sm:text-3xl">Ready to modernize your warehouse?</h3>
          <p className="mx-auto mt-2 max-w-xl text-sm opacity-80">
            Pick a plan, claim your workspace slug, and go live in minutes.
          </p>
          <button
            onClick={() => setOpen(true)}
