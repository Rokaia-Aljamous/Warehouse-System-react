...
/**
 * Supervisor transfer workflow. Transfer requests are loaded through REST
 * (`GET /{slug}/keeper/transfer-requests`, scoped to the supervisor's
 * warehouse) on mount and refreshed on a polling interval, so the feed works
 * even when realtime broadcasting is unavailable. The owner-scoped realtime
 * channel (`transfer.requests.{ownerId}`) acts only as a live incremental
 * source on top of the REST-loaded data. Preparation and delivery tasks are
 * read/assigned exclusively through the real keeper task APIs
 * (`GET|POST /{slug}/keeper/tasks*`).
 */
export function useSupervisorTransfers(
  ...
  const refreshRequests = useCallback(async () => {
    if (!slug) return;
    try {
      const res = await fetchKeeperTransferRequests(slug);
      setRequests(res.transfer_requests);
    } catch (err: unknown) {
      const status = (err as { response?: { status?: number } })?.response?.status;
      if (status !== 401 && status !== 403) {
        console.error("fetch keeper transfer requests failed", err);
      }
    } finally {
  ...
  /* REST fallback + polling: initial authoritative feed, kept fresh even when
     realtime broadcasting is unavailable. */
  useEffect(() => {
    refreshRequests();
  ...
  /* Realtime discovery: the accepted transfer request reaches this warehouse's
     ...
     warehouse_secretary role). Additive on top of the REST-loaded feed. */
  useEffect(() => {
